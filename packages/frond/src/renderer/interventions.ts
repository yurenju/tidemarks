...
export const INTERVENTIONS: readonly Intervention[] = [
  ...
  },
  {
    ...
    reason: "frond-own-layer",
    why: "page turning is the consumer's to drive (ADR-0002) and this is the only declaration that says so. With the initial value a sideways finger is taken as a pan first: measured on a touch device, pointerdown, one pointermove, then pointercancel, so a consumer dragging the page along with the finger gets one frame and then nothing (yurenju/folis#140). The document is not scrollable by hand in either direction — overflow: hidden is on the same rule — so nothing is being taken away from the reader, and no book in the sample declares touch-action at all. none rather than pan-y: a vertical book paginates along the block axis, and leaving one axis to the browser lets it scroll a 直排 book off frond's own page grid (yurenju/folis#124)",
    where: "src/renderer/layout.ts",
    onlyWhenReaderOverrides: false,
  },
  {
