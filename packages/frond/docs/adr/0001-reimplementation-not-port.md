...
**代價要講清楚，因為它是真的。** 作者側判讀**不擋任何東西**；實作者與判讀者是同一個 agent，等於自己批改自己的作業；而且它只在有人開 PR 時發生，**不守回歸**。這一層是「唯一抓得到書寫方向渲染錯誤這類缺陷」的那一層，所以那個缺陷類別在回歸上沒有自動化的守門員——守它的是「每一次動到版面的 PR 都照規則跑過」這個習慣。要收回這個取捨，補的形式是把判讀搬進 CI，不是把 PR 規則寫鬆。
