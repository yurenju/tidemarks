// The one outbound vendor Folis has, and the way to run without it.
//
...
// the last step changes. That is what keeps a self-hosted Folis free of vendors, and it is
...
export function magicCodeMail(code: string): Mail {
  ...
  return {
    subject: `Folis 登入碼：${code}`,
    text: [
      `登入碼：${code}`,
      "",
      `${minutes} 分鐘內有效，只能用一次。`,
      "",
      "Folis 不會有任何人跟你要這串碼。有人這樣要求，那個人不是我們。",
...
export function loginNoticeMail(): Mail {
  return {
    subject: "Folis：剛才有人用登入碼登入",
    text: [
      "剛才有人用登入碼登入了你的 Folis 帳號。",
      "",
      "是你的話，這封信不用理會。",
