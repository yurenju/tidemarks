...
describe("parseAcceptLanguage", () => {
  ...
  // decides.
  test("puts the quality values in charge of the order", () => {
  ...
  test("keeps the written order where the qualities tie", () => {
    expect(parseAcceptLanguage("en;q=0.9,ja;q=0.9")).toEqual(["en", "ja"]);
  });

  test("ignores the wildcard, which asks for nothing in particular", () => {
    expect(parseAcceptLanguage("ja,*;q=0.5")).toEqual(["ja"]);
  });

  test("survives whitespace, empty entries and a missing header", () => {
    expect(parseAcceptLanguage(" ja ,, en ")).toEqual(["ja", "en"]);
    expect(parseAcceptLanguage("")).toEqual([]);
    expect(parseAcceptLanguage(null)).toEqual([]);
  });

  test("feeds matchLocale, which is the only reason it exists", () => {
