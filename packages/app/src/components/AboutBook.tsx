 ...
 * which is what a panel is for.
 *
 * **The contents, not the shell.** It used to be called `AboutDrawer`, back when the shell it
 * wears was a second component with a name of its own; there is one shell now (ADR-0046) and
 * this is one of the four things poured into it. What makes it the one that covers everything is
 * `needs="nothing"`, and that is not a special case made for it — it is the answer [[Contents]] and
 * [[Notes]] also get on a hand-held.
 */
export default function AboutBook({
  ...
  /** The book to describe, or `null` when this panel is not the one the hash names. */
  bookId: string | null;
  onClose: () => void;
  ...
  return (
    <Panel
      open={bookId !== null}
      onClose={onClose}
      // Three numbers and a way out, and what they describe is *a book* — which is not
      // necessarily the one on the screen underneath, since this opens from the shelf too.
      needs="nothing"
      title={
        details?.book.title ??
        msg({
          ...
          comment:
            "Title of the book-details panel, used only in the moment before the book itself has loaded — after that the panel wears the book's own title. Shares its entry with the reader's bar button that opens it.",
        })
      }
      testId="panel-about"
    >
          ...
          <section className="panel-section" data-testid="about-numbers">
            ...
            <ul className="panel-rows">
              ...
              {details.percentage !== null && (
                <li>
                  <span>
                    <Trans comment="Label of the how-far number in the book-details panel. The value beside it is a percentage.">
                      Read to
                    </Trans>
                  </span>
                  <span className="panel-note" data-testid="about-percentage">
              ...
              <li>
                <span>
                  <Trans comment="Label of the total-reading-time number in the book-details panel.">
                    Time spent
                  </Trans>
                </span>
                <span className="panel-note" data-testid="about-reading-time">
                  {formatDuration(details.readingMs)}
                </span>
              </li>
              <li>
                <span>
                  <Trans comment="Label of the how-many-times number in the book-details panel. A sitting is one continuous stretch of reading.">
                    Sittings
                  </Trans>
                </span>
                <span className="panel-note" data-testid="about-sessions">
                  <Plural
                    value={sittings}
                    one="# sitting"
                    other="# sittings"
                    comment="The how-many-times number in the book-details panel, with its unit. A sitting is one continuous stretch of reading."
                  />
                </span>
              </li>
            </ul>
 ...
 * `height: 100dvh; overflow: hidden` onto `<body>`. This panel opens over the reader as well
  ...
  return (
    <Dialog.Root modal="trap-focus" disablePointerDismissal>
      <Dialog.Trigger className="danger" data-testid="about-delete">
        <Trans comment="Button in the book-details panel that opens the delete confirmation. Shares its entry with the confirming button inside that dialog.">
          Delete
        </Trans>
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="alert-popup" role="alertdialog" data-testid="delete-confirm">
          ...
          <Dialog.Description className="panel-note">
