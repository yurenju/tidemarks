...
碰到「這件 UI 的事在 app 做起來很麻煩，是不是該搬進 frond」，**先問麻煩的原因**：
...
不是 app 的性質：測試那層會找不到自己寫的字串，巡檢那層則是產出來的圖跨機器跨日期不能比，而可比性
...
說明。**截圖預設只做 chromium 一家**，只有改動碰到 `packages/frond/src/renderer/` 或直排相關的東西才三家
都要截圖（[ADR-0039](docs/adr/0039-three-engines-are-ci-s-job-not-the-local-loop-s.md)）。
