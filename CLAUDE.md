...
⚠️ **新的 ADR 取代舊的決定時，同一批就把舊的那份刪掉**，需要的脈絡用最多三行敘述寫進新的那份，
編號留洞不要重編。這條 2026-08-28 換掉了原本的「ADR 是有日期的紀錄，內文與檔名一律不改」——那條的
原意是不要竄改當時說過的話，代價卻是每一份新的 ADR 都要把舊的複述一遍才講得清楚差在哪，脈絡因此
越疊越厚。歷史留在 `git log` 裡，那裡本來就查得到。
...
被擋一次要多花一輪去改寫，量到的是一場四次、另一場三次。**寫成 scratchpad 裡的一支 `.sh` 再 `bash`
它**，一次解決，而且下一次要改參數重跑也只要改那個檔案。
...
⚠️ **不要用 `sleep N` 去等一個背景任務。** 三場 flaky 調查逐筆量過時間，其中**兩場**這是最大的一塊
損失：一場的 11 個背景任務平均每趟多睡 195 秒，合計 35 分 43 秒、佔那場工作時長的 **27%**；另一場的
`sleep` 輪詢吃掉工具時間的 **53%**。那兩場的「最貴的前 15 個指令」裡，`sleep` 各佔 14 個與 10 個。
（第三場幾乎沒有這個問題，因為它一開始就把跑法寫成腳本、輸出寫檔——那就是下面要說的做法。）

**跑得完就前景跑。** 前景的上限是 10 分鐘，而上面那 11 個背景任務有 **10 個實跑不到 8 分鐘**——前景
會在它結束的那一秒回來，`sleep` 多付的全是尾巴。narrow 那支 21 秒、單一引擎一支 spec 幾十秒到兩三
分鐘，都在前景的範圍裡。

⚠️ 真的超過 10 分鐘才丟背景，而**查 flaky 常常真的超過**：整套 × 三個並行容器一趟就要五到十分鐘，
量到過剛好 10.0 分的一趟。那種的丟背景，然後**等它自己通知**，不要輪詢。

⚠️ **`sleep 900; tail …` 這種寫法會整個白做。** `sleep` 撞上工具自己的 10 分鐘上限被砍（exit 143），
分號後面的 `tail` 一次都沒跑到——量到兩次，兩格共 20 分鐘拿回零位元組，然後還要再補一次 `tail`。
