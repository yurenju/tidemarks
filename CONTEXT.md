...
三條硬規則：直角是預設（只有被手指捏住的東西是圓的）；陰影 blur 永遠 0，而且**有影子＝這個東西
...
灰的（`--text-muted`，永遠不用綠）、固定 3px、沒有 thumb、沒有端點、`pointer-events: none`、
...
形態亮暗一致：一塊底加一條下緣的筆壓線。那條線是暗色下還讀得到的原因，不是裝飾。
