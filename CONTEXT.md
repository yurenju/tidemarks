...
多的是一次點擊，省下的是書架第一屏上一整個寫字的區塊。留一個空輸入框在書架上，等於在那一屏放了一張
欠填的表。
...
就欠著，然後讀者開始躲開放它的那一屏。換一則是讀者主動要，不按不會累積任何東西。
...
**`lastShownAt` 還在寫，只是沒有人讀。** 它是那個排序唯一需要的欄位，而一個停止寫入的欄位，等到要用
的那天手上是一段空白的歷史。它跟著畫線同步（「我看過這句了」是**讀者**的事實）；「今天是哪一條」不
同步，那是這台螢幕的事實。
