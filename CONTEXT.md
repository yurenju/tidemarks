...
**靠形狀分不靠顏色分**：一個是字邊 2px 的波浪線，一個是實心的控制項。
...
形態亮暗一致：**只有一條 2px 的波浪線，沒有底色**——潮水退去留下的痕跡，全 app 唯一的裝飾，
而且只出現在屬於讀者的東西上。橫排走字底下、直排走字右邊（傍線的位置）；兩邊的擺法不一樣，因為
frond 回報的矩形不一樣，量法與數字寫在 `index.css`。
