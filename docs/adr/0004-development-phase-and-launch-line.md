...
- **換掉產品名。** `Folis` 是暫名，`OPEN_SIGNUP` 打開之前要換完，理由與代價見
  [ADR-0025](0025-folis-is-a-placeholder-name-until-launch.md)。改名要動的包括網域與
  WebAuthn 的 RP ID，而 RP ID 換了每一把 passkey 都會失效——這件事**只有在使用者還是維護者
  一個人的時候做得起**。

這張清單現在只有一條，是因為只有這一條確認過。其他候選（Stripe 從 test mode 切正式、
`packages/site` 的四份法律文件、開源那天的 public repo）**還沒有被決定成上線的前置條件**，
所以不列。要加就在這裡加，不要另開一份。
