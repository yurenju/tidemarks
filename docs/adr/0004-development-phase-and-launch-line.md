...
**現在只有一條：把付費模型做進程式**（#186〈付費模型改成「帳號免費、額度收費」，程式還沒跟上〉），
2026-09-02 加的。那張票列的 1 到 7 步做完（D1 記得額度、Worker 檢查額度、client 不送沒額度的書、
書架與〈帳號〉抽屜、MCP 看不到凍結的書、降級與重新訂閱），免費帳號就能開放給任何人，那一刻就
上線。**金流不在裡面**：帳號免費，付費那一級上線之後再加
（[ADR-0011](0011-the-account-is-free-and-the-quota-is-paid.md)）。

之前列過的一條已經做完：換掉暫用的產品名，在 2026-08-19 定名、隨後改完了
（[ADR-0019](0019-the-app-is-called-tidemarks.md)）。它會列在這裡是因為改名要動 WebAuthn 的
...
（email 註冊、三本額度的檢查），做齊了才開放；付款不在裡面，帳號免費，付費那一級上線之後再加
