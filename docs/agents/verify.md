...
`?select=` 收兩種寫法，跟 `?at=` 一樣是開書那一刻讀一次：

| 寫法 | 意思 |
| --- | --- |
| 一段文字 | 在**畫面上那一節**裡找它，找到第一個就選起來 |
| `epubcfi(…)` | 精確到字。這一種不必另外給 `?at=`，它會自己跳過去 |

文字那種**不會翻遍整本書**，只找當前這一節，所以要搭一個 `?at=` 說在哪一章。找不到的時候書照常開，
原因寫在 console 裡。

⚠️ **一節通常不只一頁，而選取不會把書翻過去。** 要選的那段如果在同一節但別頁，它照樣選得起來，顏色列
也會浮出來，只是位置落在畫面外，截出來的圖看起來就像壞了。`?at=` 說的才是停在哪一頁，所以兩個要指向
同一個地方。

跳與選是兩件事，`?at=` 說跳到哪、`?select=` 說選哪一段。兩個都給的時候以 `?at=` 為準，所以「跳到章首、
選中間某一段」是寫得出來的。

⚠️ **預設是瀏覽器原生的選取，所以手機尺寸的視窗上截出來的圖沒有那兩顆把手。** Tidemarks 在觸控上是
自己畫選取的（[ADR-0036](../adr/0036-touch-selection-is-ours-not-the-browsers.md)），而
`?select=` 預設強制走原生那一套，不管視窗多大；不這樣的話手機尺寸下會什麼都不發生，畫面上也沒有東西
說明原因。那種圖可以拿來看排版，**不能拿來看選取本身長什麼樣**。要那兩顆把手就加 `&handles=1`：

```bash
playwright-cli goto "http://localhost:5001/#/book/$BOOK_ID?at=chars:12&select=…&handles=1"
```

⚠️ **強制原生只撐到下一次碰畫面為止。** 手指或滑鼠碰一下就會切回另一套，所以套上選取之後不要再 tap，
否則手機尺寸下選取會靜靜地消失。

到了沒有，一樣看 `.reader` 的 `data-at`：選取是在所有跳轉都落地之後才套上去的，所以 `arrived` 也涵蓋
它。
