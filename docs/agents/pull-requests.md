...
最終的那組數字**。最極端的一份寫過 280 行、兩套證據，而先出現的那一套已經被後面那一套推翻了。

四條：

1. **第一段先補前提，再講結論。** ⚠️ **不要寫成「第一句就是結論」**，結論幾乎都是用術語寫的
   （ADR 給的名字、自己取的元件名、上一張票的編號），而術語正是新讀者缺的那個東西。踩過一次的
   寫法是「〈別處的位置〉的提示在桌機幾乎不會出現」，〈別處的位置〉是 ADR-0037 取的名字，沒讀過
   那份 ADR 的人不知道那是什麼。要先花一兩句講**那個東西是什麼、平常怎麼運作**，再說它壞在哪。
...
**數字只留有話要說的那幾個**：推翻了什麼、或主張了什麼。量過一次的比例是兩張座標表十幾格裡只有
...
看了截圖再改一輪是好事。但**證據不要用追加的方式長第二套**：外面
...
直式與橫式各一輪。** 桌機尺寸的圖對 touch-only 的功能等於沒用，那些元素根本不會出現。做法見
〈截圖怎麼產〉的 `--device`。

**幾家瀏覽器：預設一家。** 截圖只做 chromium 一家，只有**這次改動碰到 `packages/frond/src/renderer/` 或
直排相關的東西**才三家都要截圖
...
**真的三家都截圖的那種 PR（動到渲染層），判讀要按瀏覽器分開寫**，哪一家出現哪一項本身就是資訊，三家
不一致比三家一起壞更常見，那也正是那種 PR 要三家都截圖的全部理由。那時候寫的是「三家皆跑過，五項皆
無」，沒有一家可以留空。
...
骨架長這樣。**把瀏覽器弄到「可以截圖」的那幾行 `source` 進來，中間那段每次現寫**：
...
瀏覽器、釘語言、把書放進去、打開它。要截什麼圖仍然每次現寫，因為那正是這件事的內容。**這不是把證據
...
resize。那組 setup 跑一遍要幾十秒，三家引擎都跑的話兩分鐘上下，而它跟你想量什麼、想截什麼圖完全無關。

實際發生過的事：一支 `shots.sh` 截完圖（122 秒），接著另寫一支 `measure2.sh` 去量數字（137 秒），
