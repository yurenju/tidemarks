...
所以「壓低 `workers` 就綠了」這個推論有一個對稱的版本：**「跑整套是綠的」也不能拿來結案**。忙的機器
...
換掉」那份自己另一句「把一個既有的競態放大到看得見」才是對的。留著是為了記得這種推論有多容易被寫成原因。
...
最後兩欄**允許寫「未定」**，那也是資訊。但**只填放大器不填原因的不算填完**，那正是上面說的那種推論。
...
查 flake 常常要動到量測的那一半：取樣器、trace helper、instrumented logging。**動它之前先問：有沒有一個
一行的檢查，能在寫完整版之前就告訴我這個方向是錯的？**

量到的代價是這樣的：一場調查有將近三成的時間花在兩條最後被推翻的路徑上，其中最貴的一條是重寫
量測工具（37.6 分鐘），而打敗它的事實：「一整批 mutation 會合併成一次 callback，所以翻頁在單一
frame 內完成時，中間值全被覆寫」，**印一次 `trace.offsets.length` 就知道**。

有一個訊號可以拿來自我檢查：**那段「為什麼這樣量才對」的說明，同一段寫了三遍**。一直改不定通常
表示還沒想清楚，而不是文字不夠好。這時候該回頭做那個一行的檢查。
