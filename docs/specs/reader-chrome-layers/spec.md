...
> ⚠️ **2026-09-08：上面那句「不是執行期的正本」在 Android 上只對了一半，而 `theme_color` 也不再
> 留著了。** 裝起來的 PWA 在 Android 上把上緣切成**兩塊**，各由不同的東西決定，而這一節只涵蓋了
> 其中一塊：
>
> | 區域 | 由誰決定 |
> | --- | --- |
> | 系統狀態列的**底色**（時鐘、電池那條） | manifest 的 `theme_color`；沒有這一項時改由**系統**的深淺色決定 |
> | 狀態列的**圖示**是深是淺 | 底色來自 manifest 時跟著 manifest；否則跟著 `<meta name="theme-color">` 的明暗 |
> | 狀態列正下方那條細帶 | `<meta name="theme-color">`，執行期改也算數 |
>
> 實測方式是四個探測頁，讓 manifest、meta、執行期寫入各講一個差很遠的顏色，逐項裝成 PWA 驗。
>
> 所以第四節那個 effect **沒有空轉**：細帶一直是它在管，而細帶跟它底下那一塊同色時看不出來，正是
> 這一節要的效果。它還多做了一件當初沒有人知道的事：決定狀態列的圖示畫成深色還是淺色。
>
> **決定：manifest 不再宣告 `theme_color`。** 它只給得起一個顏色（能改變這件事的
> `user_preferences.color_scheme_dark` 2021 年過了 W3C TAG review 之後就沒有下文），而寫淺色會讓
> 深色主題看起來壞掉、寫深色只是把同一個毛病換到淺色主題身上。拿掉之後底色改跟系統走，而讀者的
> 主題預設就是跟隨系統，所以狀態列變成跟頁面差一點點，而不是相反。
>
> ⚠️ **代價寫在這裡，是知情下接受的**：底色來自系統、圖示來自 meta 的明暗，兩個來源在「讀者手動
> 選了跟系統相反的主題」時會打架，結果是白底白圖示，時鐘與電量看不見。接受的理由是那條接縫每次
> 打開 app 都會看到，而圖示不見只發生在讀者自己選出來、看得到、也改得回去的那個狀態裡。
>
> `background_color` 不動，它管的是啟動畫面，原本那條「icon 畫在紙上」的理由照舊成立。
>
> 影響範圍只有 Android：桌面 Chromium 的視窗標題列與 iOS 的狀態列看的都是頁面 HTML 裡的 meta，
> 本來就跟著主題走。
...
> ⚠️ **2026-09-08：上面最後一條驗收條件的措辭給錯了保證。** 它只問「不是黑的」，而當時 Android 的
> 系統狀態列吃的是 manifest 裡寫死的紙色（見第四節的更新），所以這條一直是綠的，卻沒有守住任何
> 東西，因為它問的那件事無論如何都會成立。
>
> 換成兩條，都在真機的 PWA 上手動看：
>
> - **主題跟隨系統時**，狀態列與頁面只差一點色調，沒有明顯的一條帶。
> - **手動選成跟系統相反時**，狀態列的時鐘與電量看不見。這是**已知且接受的**，不是回歸。
>
> 這件事沒有自動化的接縫可以測：狀態列由 Android 畫，不在 viewport 裡，Playwright 看不到，容器裡
> 也裝不出 PWA。細帶那一半有接縫，`chrome-layers.spec.ts` 已經在守著（它斷言的是 meta 的內容）。
