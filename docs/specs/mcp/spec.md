...
- **讓 renderer 在 headless DOM 上跑。** 買不到東西：CFI 定位不需要排版，而真的需要排版的（分頁、矩形）headless DOM 也給不了。代價是引入 runtime 相依，而 frond 是零 runtime 相依的。
- **只讓 agent 錨在當前頁**（client 把整頁的段落清單連同各段 CFI 一起送上來）。這條真的可行、而且便宜，一度是首選；被換掉是因為它把能力留在錯的層——agent 讀得到全書卻只錨得到一頁，而那條界線是實作限制長出來的，不是設計想要的。
