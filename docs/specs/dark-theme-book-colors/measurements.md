...
> ⚠️ **2026-08-28：底下的對比數字量的是一個已經換掉的背景。** 深色主題的紙從 `#1b1b1e` 換成
> `#16202b`（`--surface-page`，跟閱讀器外框同一個值），常數也從 `DARK_THEME` 改叫
> `BOOK_THEMES`。**分佈與那兩個亮度門檻不受影響**，它們量的是書自己宣告的顏色，跟背景無關；
> 受影響的只有「對比多少」那幾欄，要重驗就照〈怎麼重驗〉那節換背景再跑一次。原文一律不改。
