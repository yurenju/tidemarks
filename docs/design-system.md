...
1. **這是紙、墨、還是潮？**紙是容器（讀者改不動），墨是讀者留下的痕跡（重點與筆記），潮是可以動的。
...
| token | 亮 | 暗 | 用在 |
| --- | --- | --- | --- |
| `--surface-raised` | `#fbf7ed` | `#1b2733` | 浮起來的：卡、彈窗、蓋住整個畫面的面板、閱讀器的三條 bar |
| `--surface-page` | `#f4eee2` | `#16202b` | 書頁、底、按鈕與欄位的底 |
| `--surface-sunken` | `#ede5d6` | `#101a24` | 凹進去的：站在書旁邊那一欄的面板、唯讀進度條的軌 |
...
⚠️ **「墨」這個材質名專指讀者留下的東西**（見〈重點〉）。正文的四階濃度是另一件事，規則沒變：
...
**沒有綠色。**「已同步／成功」用 `--tide`。（重點的苔綠是書上的顏色，不是介面的，見〈重點〉。）
...
| `--type-title` | 1.375rem | 同左 | ui | 現在沒有畫面用它（大書拆掉之後空出來的一階） |
| `--type-lede` | 1.125rem | 1.1875rem | ui | 確認框標題、面板角落的 ✕、〈帳號〉第一句、〈回訪卡〉的引文 |
...
行高有兩個，兩個都在 handoff 的白名單（1.0 / 1.4 / 1.5 / 1.8 / 2.0）裡：`--leading-text` **1.8**
...
⚠️ **不要照選項數量與字長來挑**（那是舊的依據，已經換掉）。先問這組值是什麼
（[ADR-0050](adr/0050-a-setting-lives-where-you-can-see-it-work.md)）：

| 那組值是 | 用什麼 | 例子 |
| --- | --- | --- |
| 並列的選項，而且畫得出來 | **磚（tile）** | 主題、欄數、字型 |
| 並列的選項，畫不出來 | **segmented** | 介面語言（三個名字各寫成自己那種語言，那排字本身就是預覽） |
| 一條刻度，值有順序 | **stepper** | 行距、留白 |
| 連續的量 | 滑桿 | 字級 |

版面一律**標籤在上、控制項在下**，每一列靠左對齊同一條線。混形狀的表單靠的是那條共同的左緣，
不是靠每個控制項都同一種形狀。標籤比控制項小一級（`--type-note`）且是 `--text-muted`，不然它在
自己那一行上會讀成一個章節標題。

### segmented

**選項少又短、又畫不出來的設定，就把選項全攤在畫面上**，不要藏進 `<select>` 裡點開才看得到。
...
一排等寬的方塊，每一塊**畫出它自己**：字型的磚用那支字寫 `Aa`，雙欄的磚畫兩欄，「跟隨系統」的磚畫
一顆半黑半白的圓。
磚比 segmented 高兩三倍，換到的就是那張圖。**畫不出東西的設定不要用磚**，那只是加了裝飾的按鈕。

- 一排均分，所以每塊一樣大，**跟標籤多長無關**。圖只有在框一樣大的時候才讀得出是在比較。
- ⚠️ **兩個設定不能用同一張圖。** 〈主題〉的「跟隨系統」與〈欄數〉的「自動」都是「你決定」，
  第一版兩張都是半黑半白的圓，那正是磚會出而文字格子不會出的錯。
- **選中＝`--fill-selected` 加 `--tide` 的邊與字，不是實心。** 這是上面那條「選中＝實心」的例外，
  理由是面積：一格 segment 一個字高，塗滿仍然讀成一個字；一塊磚是三倍，塗滿就是一塊色塊。
- 圖用 `--text-primary`（選中時 `--tide`），字用 `--text-muted`：圖是被選的東西，字是它的說明。
- disabled 與 tab stop 的規則跟 segmented 一樣，它們本來就是同一個元件的兩種外觀。

### stepper

`−`、目前的值、`+`，一列。給的是**刻度**：讀者問的是「比現在多還是少」，而不是「這六個裡哪一個」。

- 三格共用邊、**同一個高度**（值那格要 `align-items: stretch`，不然它只長到文字高，比兩顆鈕矮 15px）。
- **兩端要 disable，不繞回去。** 並列的選項繞回去沒有損失，刻度繞回去等於讀者按「更窄」拿到最寬。
- `role="spinbutton"` 在值那一格上，帶 tab stop 與方向鍵；兩顆鈕是手指按同一件事的方式，
  所以它們的 `aria-label` 講方向（Less／More）而不是講值。
- **按到底的那顆鈕不能繼續握著 focus。** 一顆按到底就會 disable，而 disable 一個正握著 focus 的按鈕，
  焦點會掉到 `<body>`，下一次方向鍵就跑去翻頁，把讀者正在調的那張面板帶走。按到端點時把 focus
  交給值那一格。
...
觸控上選字是我們自己畫的（[ADR-0036](adr/0036-touch-selection-is-ours-not-the-browsers.md)），所以
反白、兩個端點手把、顏色列的讓位都是這一層的數字。**桌機留原生選取**，只有底色跟著換。

| 東西 | 數字 | 為什麼是這個 |
| --- | --- | --- |
| 反白底 | `--selection-wash`（潮色 16%／暗色 22%） | 一塊底，不是波浪線，波浪線是〈重點〉的形態，而那是墨。淡到不糊 CJK 的筆畫 |
| 手把珠子 | 11px 圓 | 圓的是兩個手機系統的肌肉記憶。15px 在中文的行距下會壓到隔壁行 |
| 手把柄 | 2×9px，貼著選取邊緣 | 珠子往外掛，柄指回它屬於的那一端 |
| 手把 hit 區 | 44px（`--tap-min`） | 跟全站可按的東西同一個下限 |
| 顏色列的 `gap` | 24px | 見下 |

**看得到 11px、摸得到 44px，是兩組框。** 跟〈重點〉的畫框／點框同一種分法：畫出來的東西要小到不擋
字，摸得到的範圍要大到手指按得中。

**`placeSelectionToolbar` 的 `gap` 從 8 加到 24，是為了讓開手把。** 8px 是原生選取年代的數字，那時候
選取邊緣什麼都沒有；現在手把的柄從邊緣往外 13px、珠子再 11px，8px 會讓顏色列壓在下面那個手把上，
壓住的還不只是看得到的珠子，是那個 44px 的 hit 區，症狀是「有一端拖得動、另一端拖不動，而且看不出
為什麼」。**桌機也跟著加大**：桌機沒有手把，多出來的 16px 看不出差別，而兩邊同一個數字就不會有人
日後只改一邊。

**直排時整組轉九十度**：手把掛在字的左右兩側而不是行的上下，顏色列也已經是坐在選取旁邊（不是下面）。
兩件事同一個理由：直排的一段選取是往左右展開的，擺在「下面」就落在整段的正下方、還擋住還沒選的行。

### 圖示

不引圖示字型、也不用線條式的圖示。全站只有五個字元：`⋯`（詳情）`✕`（關閉）`‹`（回書架，桌機的上一頁鈕
也是它，下一頁是鏡射的 `›`）`⌄`（排序，目前由原生 `<select>` 自己畫），加上重點色票的圓。每個都在
44×44 的方框裡、無底色。
...
書的狀態固定**兩行**：第一行位置、第二行時間感。**那兩行是牆上卡片的形狀。**〈正在讀〉那一排只帶
第一行（位置），而且 600 以下連那一行都不帶，一排上兩句話會變成一句話加半句話，而時間感本來就有
自己的一行在牆上。算不出來就換一句成立的話，**不留空行**。四種說法
