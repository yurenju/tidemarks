...
| token | 亮 | 暗 | 用在 |
| --- | --- | --- | --- |
| `--quote-ink` | `#9aa8b8` | `#4a6478` | 〈回訪卡〉引文前面那個大引號，全站只有這一處 |

**它為什麼不是既有的任何一個。** 不是 `--text-faint`：那一階是線與 disabled，而這個是一個字形。
不是 `--tide`：潮色的意思是「這個可以按」，而引號按不動，潮色出現的地方要數得出來（〈怎麼判斷〉）。
也不是 `--mark-indigo`：那是讀者留下的顏色，而這個引號是卡片在引用，不是讀者畫的。

**它不帶字**，所以沒有文字對比的門檻要過——它比內文淡是刻意的，讀者要先讀到句子而不是先讀到引號。
上面那條「一個沒有人用的顏色名比一個缺口更糟」在這裡是反過來成立的：有一個地方真的需要它。
...
| `--type-lede` | 1.125rem | 1.1875rem | ui | 抽屜標題、確認框標題、〈帳號〉第一句、回訪卡的出處 |
...
| token | 值 | 用在 |
| --- | --- | --- |
| `--mark-quote-lines` | 5 | 〈回訪卡〉的引文封頂幾行 |

它是**行數不是像素**，而那是規則不是偏好：讀者可以把瀏覽器字級調大（[ADR-0006](adr/0006-font-size-is-a-percentage-of-the-readers-root.md)），
寫死像素的話那時候裝得下的行數就會縮水。它同時是**地板也是天花板**——卡片一次顯示五條畫線的其中
一條，高度會變的話按鈕就會在讀者翻的時候從拇指底下移開。

同一張卡上「想法」那一塊也是封頂三行，但**它沒有 token**：那是一個 `<textarea rows={3}>`，而
textarea 的 `rows` 本來就是用自己的行數在算。以前有一個 `--mark-note-lines`，花在一條還要自己
把內距和兩條髮絲線加回去的 `height` 算式上。

⚠️ 五行**確實會切掉長詩**，那是為了讓〈正在讀〉留在手機第一屏付的價，取捨寫在
[ADR-0038](adr/0038-the-revisit-card-remembers-what-it-showed.md)。要動它們之前先讀那一段。

行高有三個，三個都在 handoff 的白名單（1.0 / 1.4 / 1.5 / 1.8 / 2.0）裡：`--leading-text` **1.8**
（任何由句子組成的東西）與 `--leading-title` **1.4**。中文需要鬆的那個。所有會變動的數字吃
`font-variant-numeric: tabular-nums`（寫在 `body`）。

第三個是 `--mark-quote-leading` **2.0**，只給〈回訪卡〉上的引文，寫在 `.mark-card` 上而不是
`tokens.css`——**它是那張卡自己的，不是系統的一階**。它比 1.8 鬆，理由是那段字是書的內容而不是介面
的文字，而且它被排在一張卡的寬度裡不是一頁的寬度裡。handoff 指定的是 2.1，白名單上沒有 2.1，而在
閱讀字級下兩者差 1.6px——不值得為此多一個別人拿不到的值。
...
| 東西 | 數字 | 為什麼是這個 |
| --- | --- | --- |
| 反白底 | `--selection-wash`（潮色 16%／暗色 22%） | 一塊底，不是波浪線——波浪線是〈螢光〉的形態，而那是墨。淡到不糊 CJK 的筆畫 |
| 手把珠子 | 11px 圓 | 圓的是兩個手機系統的肌肉記憶。15px 在中文的行距下會壓到隔壁行 |
| 手把柄 | 2×9px，貼著選取邊緣 | 珠子往外掛，柄指回它屬於的那一端 |
| 手把 hit 區 | 44px（`--tap-min`） | 跟全站可按的東西同一個下限 |
| 顏色列的 `gap` | 24px | 見下 |

**看得到 11px、摸得到 44px，是兩組框。** 跟〈螢光〉的畫框／點框同一種分法：畫出來的東西要小到不擋
字，摸得到的範圍要大到手指按得中。
