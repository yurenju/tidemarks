...
- tile 是 8×6（直排 6×8）：中心線走 2→4，`stroke-width: 2` 讓墨跡落在 1→5，兩邊各留 1px。
  **邊界留不夠波谷會被 SVG 切掉**，整條波浪就變成平底的——那是實際踩過的。波長從 12 縮到 8 是因為
  高度受限（見下），只能靠縮短波長讓它讀得出來是波浪。
- **兩個軸的擺法相反，而那是量出來的**：
  - **橫排的矩形是行框**（18.4px 的字回 26px），而且上下相鄰的矩形間距是 **0**。畫在框外面就是
    畫在下一行的字上，所以橫排畫在**框內**，用行框自帶的半行距。
  - **直排的矩形是字身框**（23px 的字回 11px），緊貼著字。畫在框裡面就是畫在筆畫上，所以直排畫在
    **框外**、欄與欄之間，那也正是傍線的位置。
  - 一份在普通文件裡做的原型兩個方向都猜錯了，因為裸的 `Range` 回的是字身框而 frond 不是。frond 的
    `rectsFor` 哪天改了，要重量的就是這兩個數字。
