...
| `--mark-note-lines` | 3 | 同一張卡上，想法那一塊封頂幾行 |

這兩個是**行數不是像素**，而那是規則不是偏好：讀者可以把瀏覽器字級調大（[ADR-0006](adr/0006-font-size-is-a-percentage-of-the-readers-root.md)），
寫死像素的話那時候裝得下的行數就會縮水。它們同時是**地板也是天花板**——卡片一次顯示五條畫線的其中
一條，高度會變的話 ‹ › 就會在讀者翻的時候從拇指底下移開。
